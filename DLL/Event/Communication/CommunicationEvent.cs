...
#region - [1.12] -
/*
 *  02/03/2016  1.12     DAS            References
 ...
 *                                          
 *                                      2.  Bug Fix - SNCR - R188 [20-Mar215] Item 37. Updated to support real time events (events that occur while viewing the
 ...
 *                                      
 *                                      Modifications
 *                                      1.  Replaced all calls to unmanaged code with calls to managed code since the C++ communication DLL is no longer used. - Ref.: 1.
 *                                      2.  Removed all function delegates to the unmanaged Event communication functions along with their associated member
 *                                          variables. - Ref.: 1.
 ...
 *                                          bit or 64 bit version of the event communication methods as managed code supports both 32 bit and 64 bit. - Ref.: 1.
 *                                          architecture.
 *                                      4.  Modified the signature of the CheckFaultLogger() signature to include the newEventsLogged parameter. - Ref.: 2.
 */
#endregion - [1.12] -

#region - [1.13] -
/*
 *  09/12/2016  1.13     DAS            References
 *                                      1.  Bug Fix - Support 4 digit year code returned from VCU.
 *                                      
 *                                      Modifications
 *                                      1.  Modified the call to EventStreamMarshal.GetFaultHdr() in the GetEventRecord() method to include Parameter.Use4DigitYearCode.
 *                                      2.  Modified the call to EventStreamMarshal.GetFaultVar() in the GetFaultVar() method to include Parameter.Use4DigitYearCode.
 */
#endregion - [1.13] -
...
namespace Event.Communication
{
    ...
    public class CommunicationEvent : CommunicationParent, ICommunicationEvent
    {
        ...
        public void GetEventRecord(Log currentEventLog, short eventIndex, out EventRecord eventRecord)
        {
            ...
            {
                m_MutexCommuncationInterface.WaitOne(DefaultMutexWaitDurationMs, false);
                errorCode = m_EventStreamMarshal.GetFaultHdr(eventIndex, ref eventIdentifier, ref taskIdentifier, 
                                                             ref time, ref date, ref streamNumber, Parameter.Use4DigitYearCode);
            }
            catch (Exception)
            {
                errorCode = CommunicationError.SystemException;
        ...
        public void GetFaultVar(short eventIndex, short eventVariableCount, short[] dataTypes, out double[] values)
        {
            ...
            {
                m_MutexCommuncationInterface.WaitOne(DefaultMutexWaitDurationMs, false);
                errorCode = m_EventStreamMarshal.GetFaultVar(eventIndex, eventVariableCount, dataTypes, values, Parameter.Use4DigitYearCode);
            }
            catch (Exception)
            {
                errorCode = CommunicationError.SystemException;
