 ...
 *                                          
 *  07/11/2016  1.4     K.McD           References
 ...
 *                                      
 *                                      Modifications
 *                                      1.  Modified the UpdateCount() method to include the m_LabelCountTotal Label.
 *                                      
 *
 ...
 *                                  
 *                                      Modifications
 *                                      1.  Modified the Cleanup() method to de-register the m_ListBoxAvailable_Enter() event handler from the 'Enter' event associated
 *                                          with each 'Test Category' ListBox control.
 *                                      2.  Modified the m_TabControlAvailable_SelectedIndexChanged() event handler to clear any items that were previously selected
 *                                          from the selected ListBox control.
 *                                      3.  Overrode the m_ListBoxWorkset_Enter() event handler to clear any selected items from the 'Test Categories' ListBox controls.
 *                                      4.  Modified the ConstructTabPage() method to register the m_ListBoxAvailable_Enter() event handler to the 'Enter' event
 *                                          associated with the 'Test Categories' ListBox control on the constructed TabPage.
 */
...
namespace SelfTest.Forms
{
    ...
    public partial class FormTestListDefine : FormWorksetDefine
    {
            ...
            try
            {
                ...
                if (disposing)
                {
                    ...
                    for (int tabPageIndex = 0; tabPageIndex < m_TabControlAvailable.TabPages.Count; tabPageIndex++)
                    {
                        ...
                        listBox.Enter -= new System.EventHandler(this.m_ListBoxAvailable_Enter);
                        listBox.Items.Clear();
                    }
        ...
        /// <param name="sender">Reference to the object that raised the event.</param>
        /// <param name="e">Parameter passed from the object that raised the event.</param>
        protected override void m_ListBoxWorkset_Enter(object sender, EventArgs e)
        {
            TabPage tabPage;
            ListBox listBox;
            // Clear the selected items from all ListBox controls relating to the available tests.
            for (int tabPageIndex = 0; tabPageIndex < m_TabControlAvailable.TabPages.Count; tabPageIndex++)
            {
                tabPage = m_TabControlAvailable.TabPages[tabPageIndex];
                listBox = (ListBox)tabPage.Controls[KeyListBoxAvailable + tabPage.Tag.ToString()];
