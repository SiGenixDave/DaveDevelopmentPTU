 ...
 *                                          that is to be copied rather than the number of records in the current data dictionary when initializing the WatchItems
 *                                          variable.
 ...
 *                                      8.  Removed the ActiveWorksetModified event and replaced it with the WorksetCollectionModified event, this event is raised
 *                                          whenever any element of the workset collection is modified.
 * 
 ...
 *										2.	Modified CheckCompatibility() to check whether the workset has an upper limit of watch variables different from the data
 *										    dictionary.
 *										3.	Modified Update() to set the correct upper limit watch size according to the data dictionary and notify the user if doing so
 *										    will potentially lose data.
 *											
...
/*
 *  11/16/15    1.9     K.McD       References
 *                                  1.  Bug Fix - SNCR-R188 PTU [20th Mar 2015] Item 36. If the WATCHSIZE is set to 40 and the number of watch variables is less than
 *                                      40 an exception is thrown on start-up.
 *                                  
 *                                  Modifications
 *                                  1.  Added a System.IndexOutOfRangeException catch block to the try block associated with the CreateBaselineWorkset() method.
 */
    ...
    public class WorksetCollection
    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else if (index > ActiveIndex)
                            {
                                if (index < DefaultIndex)
                                {
                                    // The deleted workset index is above the active workset index but below the default workset index. Decrement the default workset
                                    // index by 1.
                                    m_WorksetCollectionFile_t.DefaultIndex = 1;
                                }
                                else if (index > m_WorksetCollectionFile_t.DefaultIndex)
        ...
        public Workset_t CreateBaselineWorkset()
        {
            ...
            for (short watchIdentifier = 0; watchElementList.Count < EntryCountMax; watchIdentifier++)
            {
                ...
                {
                    // Include this in case the number of watch variables in the data dictionary is less that EntryCountMax.
                    break;
                }
            }
