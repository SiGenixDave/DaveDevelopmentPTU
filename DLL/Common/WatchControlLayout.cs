 ...
 *                                      
 *  08/02/13    1.4     K.McD           1.  Increased the width, in pixels, of the name field associated with the watch control from 200 to 230 to prevent some of the
 *                                          longer variable names being displayed incorrectly. 
 *                                          
 *                                          In the WatchControl user control, the label associated with the variable name has the TextAlign property set to MiddleLeft
 *                                          and the AutoEllipsis property set to True. If the variable name text, when printed using the selected font, exceeds the width
 *                                          of the label the text is automatically aligned to TopLeft, truncated and an ellipsis i.e. '...' is appended to the text to
 *                                          show that it has been truncated. Although the text is still readable, it appears vertically misaligned.
 *                                          
 *  08/05/13    1.5     K.McD           1.  Changed the WidthWatchControlVariableNameField constant to 210 to optimize the size of the the Watch Window user controls for
 *                                          a display resolution of 1280 x 960.
 *
 *  07/20/2016  1.6     K.McD           References
 *                                      1.  PTE Changes - List 5-17-2016.xlsx Item 62. On all screens, change the green text to black. 
 *                                      
 *                                      Modifications
 *                                      1.  Modified the ConfigureWatchControls() method to set the ForeColorValueFieldZero and ForeColorValueFieldNonZero properties
 *                                          of the control corresponding to the rowIndex element of the WatchControls array to Color.Black.
 *
 *
 */
...
namespace Common
{
    ...
    public class WatchControlLayout
    {
        ...
        /// <exception cref="ArgumentException">Thrown if the number of watch identifier entries in the list is incompatible with the length of the user control array.
        /// </exception>
        public void ConfigureWatchControls(WatchControl[] watchControls, Panel panel, VariableControlSize_t watchControlSize, List<short> oldIdentifierList)
        {
            ...
            for (int rowIndex = 0; rowIndex < oldIdentifierList.Count; rowIndex++)
            {
                ...
                try
                {
                    ...
                    {
                        // The specified watch variable is not defined in the current data dictionary therefore display an empty user control showing the 'not-defined'
                        // text.
                ...
                watchControls[rowIndex].ForeColorValueFieldZero = Color.Black;
                watchControls[rowIndex].ForeColorValueFieldNonZero = Color.Black;
